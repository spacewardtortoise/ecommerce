...
class Command(BaseCommand):
    ...
    def number_of_payments_for_order(order):
        ...
        Returns: Total number of times order is paid.

        """
        number_of_payments = 0
        payment_processor_responses = PaymentProcessorResponse.objects.all().filter(basket_id=order.basket_id)
        ...
        for payment_processor_response in payment_processor_responses:
            ...
            if 'paypal' in response:
                ...
                elif 'update_time' or 'message' in response:
                    continue
                else:
                    number_of_payments += 1
            else:
